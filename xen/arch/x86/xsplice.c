...
void
xsplice_apply_jmp(struct xsplice_patch_func *func)
{
    uint32_t val;
    uint8_t *old_ptr;
...
void
xsplice_revert_jmp(struct xsplice_patch_func *func)
{
    memcpy((void *)func->old_addr, func->undo, PATCH_INSN_SIZE);
}

int
xsplice_verify_elf(uint8_t *data, ssize_t len)
{
...
int
xsplice_perform_rel(struct Elf *elf,
                    struct Elf_Sec *base,
                    struct Elf_Sec *rela)
{
...
int
xsplice_perform_rela(struct Elf *elf,
                     struct Elf_Sec *base,
                     struct Elf_Sec *rela)
{
    Elf64_Rela *r;
